                    ...
                    @if(count($user->getExamList(['branch' => $branch])) > 0)
                        ...
                        <div class="content">
                            @foreach($user->getExamList(['branch' => $branch]) as $exam => $gradeInfo)
                                <div class="row">
                                    <div class="small-5 columns Incised901Light ninety textLeft">{{$exam}} @if (!is_null(ExamList::where('exam_id','=',$exam)->first()->name)){{ExamList::where('exam_id','=',$exam)->first()->name}}@endif</div>
