...
@section('content')
    ...
    <table id="announcementList" class="compact row-border stripe" width="75%">
        ...
        @foreach($announcements as $announcement)
            <tr>
                <td>
                    <a href="{!! route('announcement.edit', [ $announcement->id ] )!!}">{!! $announcement->summary !!}</a>
                </td>
                <td>@if( $announcement->user ){!! $announcement->user->getGreeting() . ' ' . $announcement->user->last_name !!} @else
                        Undefined @endif</td>
                <td>{!! $announcement->updated_at !!}</td>
                <td>{!! $announcement->is_published ? 'Published' : 'Unpublished' !!}</td>
                <td><a href="#" id="{!! $announcement->id !!}" class="btn"
                       onclick="confirmDelete( '{!! $announcement->id !!}' );"
                       data-title="{!! $announcement->summary !!}">Delete</a>
