<div class="ribbons">
    ...
    @foreach($user->leftRibbons as $index => $ribbon)
        ...
        @endif
        <img src="{{asset($ribbonPath)}}" alt="{!!$ribbon['name']!!}" data-toggle="tooltip"
             title="{!!$ribbon['name']!!}"
             class="ribbon">@if($user->leftRibbonCount % $user->numAcross == $index % $user->numAcross) <br/> @endif
    @endforeach
    ...
    @if(!empty($awq) && ($user->branch == 'RMA' || $user->hasAllPermissions()))
        <div class="text-center">
            @foreach($awq as $badge)
                @if($loop->index < 3)
                    <img src="{{asset('awards/badges/' . $badge . '.svg')}}" class="awq">
                @endif
            @endforeach
        </div>
    @endif
