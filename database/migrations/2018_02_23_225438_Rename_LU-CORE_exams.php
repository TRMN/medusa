...
class RenameLUCOREExams extends Migration
{
    ...
    public function up()
    {
        ...
        foreach (App\Models\ExamList::where('exam_id', 'like', 'LU-Core-0%')->get() as $exam) {
            $exam->exam_id = str_replace('LU-Core', 'SKU-CORE', $exam->exam_id);
            $exam->save();
        }

        // Change all the exam records
        foreach (App\Models\Exam::all() as $record) {
            $exams = $record->exams;
    ...
    {
        // Change the exam_id in the exam_list
        foreach (App\Models\ExamList::where('exam_id', 'like', 'SKU-CORE-0%')->get() as
                 $exam) {
